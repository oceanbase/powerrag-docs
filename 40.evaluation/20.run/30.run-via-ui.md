...
您可以在 AgentOps 中通过控制台跑测来测试来自[提示词管理](/30.prompt/00.prompt-overview.md)或语言模型的不同提示词版本，并且并排比较结果。

或者，您可以使用 [LLM 作为评判评估器](/40.evaluation/10.evaluation-method/00.llm-as-a-judge.md)根据预期输出自动对响应进行评分，以便进一步在总体层面上分析结果。
...
## 前提条件
### 创建可用的提示词
创建您想要测试和评估的提示词。创建提示词，请参见 [提示词管理](/30.prompt/00.prompt-overview.md)。
...
```python
{
  "documentation": "AgentOps is an LLM Engineering Platform",
...
```python
{
  "message_history": [
    ...
    {
      "role": "assistant",
      "content": "AgentOps is a tool for tracking and analyzing the performance of language models."
    }
  ],
  "question": "What is AgentOps?"
}
...
### 创建可用的数据集
创建一个数据集，其中包含您想要用于即时跑测的输入和预期输出。创建数据集，请参见 [数据集](/40.evaluation/20.run/10.dataset.md)。
...
### 配置 LLM 连接
跑测前，您需要添加模型。如何添加模型，请参见 [配置 LLM 连接](/40.evaluation/30.configure-llm.md)。

### 可选：设置 LLM 作为评判
您可以设置一个 LLM 作为评判评估器，根据预期结果对答案进行评分。请确保将 LLM 作为评判的目标设置为“跑测”，并筛选您想要使用的数据集。设置  LLM 作为评判，请参见 [LLM 作为评判](//40.evaluation/10.evaluation-method/00.llm-as-a-judge.md)。
