...
5. 按需进行工作流编排后，保存智能体。具体工作流的编排操作，请参见 [RAGFlow 说明文档](https://ragflow.io/docs/v0.21.1/agent_introduction)。
...
2. 在弹窗中按需依次输入知识库名称，选择嵌入模型后，在 **Ingestion pipeline** 下选择 **选择pipeline**，然后在下拉框中选择刚刚创建的 Pipeline，并保存。
    ...
    ![2](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/powerrag/langextract/2.png)

3. 按需进行知识库配置。具体操作，请参见 [RAGFlow Datasets 说明文档](https://ragflow.io/docs/v0.21.1/configure_knowledge_base)。
4. 上传待提取的文档文件并完成解析。
...
2. 在会话的聊天设置中，进行如下配置
    1. 知识库：选择上一步中创建的知识库。
    2. 元数据：设置为 **自动**。
    3. 其他参数可根据 [RAGFlow Chat 说明文档](https://ragflow.io/docs/v0.21.1/category/chat)按需配置。
...
* 当启用元数据功能时，系统可根据已提取的结构化信息直接返回匹配结果。
  ...
  ![3](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/powerrag/langextract/3.png)

* 若禁用元数据，则因无法访问结构化字段，查询将不返回结果。  

  ![4](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/powerrag/langextract/4.png)
...
| **参数** | **类型** | **说明** |
| --- | --- | --- |
| `text_or_documents` | string | array | 待提取的文本内容。支持两种格式：<br/>+ **字符串格式**：直接提供文本内容或 URL（当`fetch_urls=true`时）。<br/>+ **数组格式**：多文档提取，每个文档包含：<br/>    - `text`（string, **必填**）：文档文本内容<br/>    - `document_id`（string, **可选**）：文档 ID，未提供时自动生成<br/>    - `additional_context`（string, **可选**）：文档的额外上下文信息 |
| `prompt_description` | string | 提取任务的描述，说明需要提取哪些类型的信息。 |
| `examples` | array | 示例数组，至少包含一个示例，每个示例包含：<br/>+ `text`（string）：示例文本<br/>+ `extractions`（array）：提取结果数组，每个提取项包含：<br/>    - `extraction_class`（string）：提取类别（如 "name", "location", "date" 等）<br/>    - `extraction_text`（string）：提取的文本内容（必须是原文片段）<br/>    - `attributes`（object, **可选**）：提取项的属性信息 |
...
| `resolver_params` | object | 无 | 解析器参数：<br/>+ `enable_fuzzy_alignment`（boolean）：是否启用模糊对齐<br/>+ `fuzzy_alignment_threshold`（float）：模糊对齐阈值 |
| `model_parameters` | object | 无 | 模型参数：<br/>+ `max_tokens`（integer）：最大 token 数 |
...
```bash
curl -X GET "http://xxx.xxx.xxx.xxx:6000/api/v1/powerrag/struct_extract/status/262134e2-6b10-4c6b-972d-c4cb66b21b16" \
  -H "Authorization: Bearer ragflow-A2MzVjZGFhYmJhMzExZjA5NjlhMDAxNj"
```
...
| `status` | string | 任务的当前状态，包括：<br/>+ `pending`：任务已提交，等待处理<br/>+ `processing`：任务正在处理中<br/>+ `success`：任务成功处理，结果可通过 `result` 字段获取<br/>+ `failed`：任务处理失败，错误信息通过 `error` 字段返回<br/>+ `not_found`：任务未找到，返回 404 错误 |
| `created_at` | string | 任务创建时间，ISO 8601 格式 |
| `updated_at` | string | 任务更新时间，ISO 8601 格式 |
| `result` | object | 当任务成功时返回，包含提取的结构化结果 |
| `extractions` | array | 提取结果数组，包含每个提取项的信息：<br/>+ `extraction_class`: 提取类别<br/>+ `extraction_text`: 提取的文本内容<br/>+ `document_id`: 文档 ID（多文档提取时）<br/>+ `attributes`: 提取项属性（如果提供） |
