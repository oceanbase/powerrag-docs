...
AgentOps 服务是应用评估与观测模块的核心组件，负责采集、分析并可视化展示模型运行的观测数据。部署完成后，用户可通过 PowerRAG 控制台访问 AgentOps 控制台。
...
# 服务访问配置
NEXTAUTH_URL="http://<AgentOps 服务地址>:3001"
...
### 执行数据迁移
AgentOps 会自动处理数据库迁移，但建议在更新前备份数据：
