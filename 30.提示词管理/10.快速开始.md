...
## 获取 API 密钥
1. 使用 PowerRAG 账号登录 PowerRAG AgentOps 控制台。
2. 从页面左下角单击 **设置** > **API 密钥**，创建并获取 API 密钥，包括公钥和私钥。

## 创建提示词
### 使用控制台
使用 AgentOps 控制台创建新提示词或更新现有提示词。
...
5. 若未选择“生产”标签，需要手动推到生产环境。在提示词详情页面左侧版本列表，单击版本后的![](https://intranetproxy.alipay.com/skylark/lark/0/2025/png/270449/1760522726076-b6afc072-e1a6-4acb-b1a7-1510911befdb.png)图标，选择 **生产**，单击 **保存**，即可推到生产环境。
...
## 使用提示词
在运行时，您可以从 AgentOps 获取最新的生产版本。关于版本和标签管理，请参见 [版本控制](../30.prompt/30.prompt-feature/00.version-control.md)。
...
## 可选：连接 AgentOps 跟踪（Tracing）
您可以将提示词连接到使用该提示词的 LLM `generation` span。此链接可让您直接在 AgentOps 控制台按提示词版本和名称跟踪指标，并查看哪个提示词表现最佳。
