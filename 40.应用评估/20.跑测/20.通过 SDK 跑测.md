...
通过 SDK 进行的跑测可用于以编程方式循环运行您的应用或提示词，使其遍历数据集，并可选择将评估方法应用于结果。您可以使用 AgentOps 上托管的数据集或本地数据集作为跑测的基础。
...
## 运行跑测的 SDK
Python 提供了用于在数据集上运行跑测的高级抽象。数据集可以是本地的，也可以托管在 AgentOps 上。建议使用 SDK 在数据集上运行跑测。
...
SDK 支持 AgentOps 上托管的数据集和本地托管的数据集。如果您使用 AgentOps 上托管的数据集进行跑测，SDK 将自动为您创建一个数据集跑测，您可以在 AgentOps 控制台中检查和比较该数据集。对于不在 AgentOps 上托管的本地数据集，AgentOps 仅支持跟踪和评分（如果使用了评估）。

### 基本用法
从最简单的跑测开始，在本地数据上测试你的任务函数。如果你已经在 AgentOps 中拥有数据集，参见 [与 AgentOps 数据集一起使用](#usage-with-langfuse-datasets) 章节。

在本地数据上运行跑测时，AgentOps 中只会创建跟踪，不会生成任何数据集跑测。每次任务执行都会创建一个单独的跟踪，以便于观测和调试。
...
#### 使用 JS/TS SDK
确保正确设置`OpenTelemetry`，以便将跟踪信息传送到 AgentOps。执行结束时务必刷新 span 处理器，以确保所有跟踪信息均已发送。
...
### 与 AgentOps 数据集一起使用
直接跑测存储在 AgentOps 中的数据集，以进行自动跟踪和比较。

使用 AgentOps 数据集时，数据集跑测会自动在 AgentOps 中创建，并可在控制台中进行比较。这可以跟踪跑测随时间的变化，并比较同一数据集上不同方法的运行情况。
...
// 重要：关闭 OpenTelemetry 来确保发送跟踪到 AgentOps
await otelSdk.shutdown();
```
...
#### 配置评估器
评估器在项目级别评估任务输出的质量。它们接收每个项目的输入、元数据、输出和预期输出，并返回评估指标，这些指标在 AgentOps 的跟踪上以评分的形式报告。
...
#### 跑测级评估器
跑测级评估器评估完整的跑测结果并计算聚合指标。在 AgentOps 数据集上跑测时，这些评分会附加到完整的数据集跑测中，以跟踪整体跑测性能。
...
### 装配您的应用
首先，我们创建应用运行器辅助函数。下一步，每个数据集项都会调用此函数。如果您使用 AgentOps 来实现生产可观测性，则无需更改应用代码。

<main id="notice" type='explain'>
    <h4>说明</h4>
    <p>对于数据集跑测，重要的是您的应用为每次跑测创建 AgentOps 跟踪，以便它们可以链接到数据集项。</p>
</main>

#### 使用 Python SDK
假设您的 LLM 应用已经装配了 AgentOps：
...
#### 使用 JS/TS SDK
请确保已设置 JS/TS SDK 以用于应用的跟踪。如果您使用 AgentOps 进行[应用观测](/20.observability/00.observability-overview.md)，则设置相同。
...
## 可选：从控制台触发 SDK 跑测
通过 SDK 设置跑测时，允许从 AgentOps 控制台触发跑测运行会很有帮助。

您需要配置一个 Webhook 来接收来自 AgentOps 的触发请求。

### 配置 Webhook
1. 从导航栏选择 **数据集**。
2. 单击要设置远程跑测触发器的数据集，进入数据集详情页面。
3. 单击页面右上角 **新建数据集跑测** 打开设置页面。
4. 在弹窗中单击 **通过 SDK/API** 下方的![](https://intranetproxy.alipay.com/skylark/lark/0/2025/png/270449/1760688111816-b1a6f3d1-5f6d-4e27-81cd-8c760de6dc66.png)图标。
5. 输入您的外部评估服务URL，该服务将在跑测触发时接收 Webhook。输入将发送到 Webhook 的默认 config，包含 PowerRAG 和桥接器的连接参数**。**  
示例 config 如下：
...
**典型工作流程**：您的 Webhook 接收请求，从 AgentOps 获取数据集，针对数据集项目运行您的应用，评估结果，并将评分作为新的跑测导入 AgentOps。
